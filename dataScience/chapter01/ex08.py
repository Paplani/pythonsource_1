...
df = pd.read_csv("dataScience/data/class_scores.csv", encoding="utf-8-sig")
print("=== 원본 데이터 (상위 10개) ===")
print(df.head(10))
...
# 3. agg(): 반별 점수에 대한 (평균, 최댓값, 최솟값, 인원수)를 한번에
...
top_scores = df.sort_values("점수", ascending=False).groupby("반").head(3)
print("\n=== 반별 최고 득점자 ===")
print(top_scores)
