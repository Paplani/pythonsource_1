...
- 전체 행이 완전히 같아야 중복인지, 특정 컬럼(응답자 ID)만 같아도 중복인지 기준을 명확히 정해야 함 -> subset 옵션이 핵심
- keep = "first"(기본값) / "last" / False 차이
"""

df = pd.read_csv("dataScience/data/survey.csv", encoding="utf-8-sig")
print(f"=== 원본 설문 응단 ({len(df)}건)")
print(df)   
...
# 2. 중복된 응답자 ID만 골라서 출력
# keep=False를 주면 "중복된 값을 가진 행은 첫 번째 것까지 포함해서 전부 True
dup_rows = df[df.duplicated(subset="응답자ID", keep=False)]
print(f"\n=== 중복 응답자 전체 내역 ({len(dup_rows)}건)===")
print(dup_rows.sort_values("응답자ID"))
print("\n")

# 3. drop_duplicated(): 중복 제거, 기본은 첫 번재 응답만 유지(keep="first")
